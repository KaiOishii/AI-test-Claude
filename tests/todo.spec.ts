...
test.describe('ToDoの基本操作', () => {
  ...
  test('ToDoの削除', async ({ page }) => {
    ...
    await listItem.getByRole('button', { name: 'del' }).click()
...
test.describe('優先度機能', () => {
  ...
  test('高優先度のToDoを追加できる', async ({ page }) => {
    ...
    await expect(listItem.locator('.sr-only').filter({ hasText: '高' })).toBeVisible()
  })
})
...
test.describe('カテゴリ管理', () => {
  ...
  test('カテゴリの追加', async ({ page }) => {
    ...
    await page.locator('aside input[placeholder="name"]').fill('仕事')
    await page.locator('aside button:has-text("↵")').click()
