...
/**
 * クエストログ（ゲーム風タスク表示）
 * 画面左上に固定表示。今日やるべきToDoを「クエスト」として並べ、
...
// 連続達成日数（ストリーク）を localStorage で管理
function bumpStreak(): number {
  if (typeof window === 'undefined') return 0
  const today = todayStr()
  const raw = localStorage.getItem('todo_quest_streak')
  ...
  localStorage.setItem('todo_quest_streak', JSON.stringify({ date: today, count }))
  return count
}
...
export default function QuestLog() {
  ...
  useEffect(() => {
    // 既定は折りたたみ（サイドバーを隠さないように）。一度開くと記憶する
    setOpen(localStorage.getItem('todo_quest_open') === 'true')
  ...
  function toggleOpen() {
    setOpen(v => {
      localStorage.setItem('todo_quest_open', String(!v))
      return !v
    })
  }
  ...
  const pending = tops.filter(t => !t.completed)
  const doneToday = tops.filter(t => t.completed && (t.updated_at ?? '').slice(0, 10) === today)
            ...
            {pending.length === 0 ? (
              <li className="px-1 py-3 text-center">
                {total > 0 ? (
                  ...
                  <span className="text-xs text-gray-400">クエストはまだありません</span>
                )}
              </li>
            ) : (
